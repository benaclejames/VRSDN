...
pub fn handshake(connection: &mut RtmpConnection) -> Result<u8, &'static str> {
    ...
    let mut buf = [0; 1];
    let c0 = match connection.stream.read_exact(&mut buf) {
        Ok(_) => {
    ...
    let c1: CS1 = match connection.stream.read_exact(&mut buf2) {
        Ok(_) =>
    ...
    connection.stream.write_all(&s0.unwrap().serialize().unwrap()).unwrap();
    connection.stream.write_all(&s1.serialize().unwrap()).unwrap();
    connection.stream.write_all(&s2.serialize().unwrap()).unwrap();

    // Now we wait for the client to send their CS2
    let mut buf3 = vec![0; 1536];
    match connection.stream.read_exact(&mut buf3) {
        Ok(_) =>
